...
/**
 * REST controller for managing {@link tech.catenate.orchestra.domain.Concerto}.
 */
@RestController
@RequestMapping("/api/concertos")
public class ConcertoResource {
    ...
    public ResponseEntity<Concerto> createConcerto(@RequestBody Concerto concerto) throws URISyntaxException {
        ...
        concerto = concertoService.save(concerto);
        return ResponseEntity.created(new URI("/api/concertos/" + concerto.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, concerto.getId().toString()))
            .body(concerto);
    }
    ...
    public ResponseEntity<Concerto> updateConcerto(
        ...
        concerto = concertoService.update(concerto);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Concerto> getConcerto(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteConcerto(@PathVariable("id") Long id) {
