...
/**
 * REST controller for managing {@link tech.catenate.orchestra.domain.Filmato}.
 */
@RestController
@RequestMapping("/api/filmatoes")
public class FilmatoResource {
    ...
    public ResponseEntity<Filmato> createFilmato(@RequestBody Filmato filmato) throws URISyntaxException {
        ...
        filmato = filmatoService.save(filmato);
        return ResponseEntity.created(new URI("/api/filmatoes/" + filmato.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, false, ENTITY_NAME, filmato.getId().toString()))
            .body(filmato);
    }
    ...
    public ResponseEntity<Filmato> updateFilmato(@PathVariable(value = "id", required = false) final Long id, @RequestBody Filmato filmato)
        ...
        filmato = filmatoService.update(filmato);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<Filmato> getFilmato(@PathVariable("id") Long id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteFilmato(@PathVariable("id") Long id) {
